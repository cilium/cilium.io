...
5. **DNS Across Clusters:** For seamless service discovery, the Kubernetes DNS service (like CoreDNS) needs to be aware of the ClusterMesh. Cilium integrates with Kubernetes DNS to ensure that a request for a global service correctly resolves to available backend pods, regardless of which cluster they reside in.
